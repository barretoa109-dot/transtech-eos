-- La v158 quedó registrada como aplicada y la función NO la tenía.
--
-- `supabase db push` anotó la versión 20260910180000 en el historial remoto
-- y después contestó "Remote database is up to date" a los dos intentos
-- siguientes, así que el cuerpo nuevo nunca llegó. Se comprobó llamando a
-- `eos_contexto_negocio` contra producción para dos usuarios: `posicion`
-- venía undefined en los dos.
--
-- No se investigó por qué el CLI se comportó así. Lo que importa es que el
-- repositorio y la base digan lo mismo, y la forma barata y segura de
-- garantizarlo es volver a declarar la función entera bajo una versión
-- nueva: `create or replace` es idempotente y no depende de qué pasó antes.
--
-- El contenido es EXACTAMENTE el de la v158, y su porqué está escrito allá.
-- Acá no hay nada nuevo salvo esta nota.
