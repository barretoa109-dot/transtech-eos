-- Lo que la rama dejó en el resultado tiene que sobrevivir.
--
-- `eos_execute_internal_effect_v64` armaba el resultado final así:
--
--     v_result := coalesce(v_command.resultado, '{}') || jsonb_build_object(…)
--
-- Esa asignación PISA lo que la rama haya puesto en `v_result`. Con cinco
-- ramas no se notaba, porque ninguna ponía nada. `CREAR_PRODUCTO` sí: deja
-- `creados` y `ya_existian`, que es lo único que permite distinguir "cargué
-- cuatro productos" de "los cuatro ya estaban en tu catálogo".
--
-- Visto en producción apenas salió la v131: se pidió agregar un producto que
-- ya existía, el precio —correctamente— no se tocó, y EOS igual contestó
-- "Los productos quedaron cargados". Es exactamente la clase de respuesta
-- que el 6 de septiembre se corrigió en el prompt: afirmar un final que no
-- ocurrió.
--
-- Un `||` de más, y el resto de la función queda igual: se genera desde el
-- archivo de la v131 en vez de transcribirse.
