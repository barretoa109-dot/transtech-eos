-- La rama de CORREGIR_MOVIMIENTO en el ejecutor.
--
-- Se genera desde el archivo de la v139 con tres cambios y nada más:
--
--   1. La acción entra en la lista de soportadas.
--   2. La rama nueva.
--   3. `executor_version` pasa a 'v148'.
--
-- El porqué está en la cabecera de
-- `20260908150000_eos_corregir_movimiento_v148.sql`: sin este verbo, un
-- error de EOS al entender un monto era permanente desde el chat.
--
-- La idempotencia es distinta a la de las demás. Las otras acciones marcan
-- la fila que crearon; ésta no crea ninguna, corrige una que ya existía. La
-- marca es el `command_id` que la corrección deja en `metadata`: un
-- reintento encuentra su propia huella y no vuelve a tocar nada.
