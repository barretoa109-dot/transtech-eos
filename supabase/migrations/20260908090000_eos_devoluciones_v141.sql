-- Una devolución no es plata que entró: es plata que dejó de salir.
--
-- ============================================================
-- LO QUE PASA HOY
-- ============================================================
--
-- Alguien compra una camisa de 200.000, la devuelve, le devuelven la plata. Con
-- lo que hay hoy eso se anota como un ingreso de 200.000, y queda:
--
--     gastos del mes  +200.000
--     ingresos del mes +200.000
--
-- Las dos columnas infladas y el neto correcto por casualidad. Y todo lo que
-- se calcula sobre los INGRESOS —capacidad de ahorro, ratio de ahorro, deuda
-- sobre ingreso— queda mal, porque esa persona no ganó 200.000.
--
-- Peor todavía en el desglose por destino: "Ropa" sigue mostrando 200.000
-- gastados en algo que no se gastó, y el presupuesto de esa categoría se
-- consume por una compra que se deshizo.
--
-- ============================================================
-- POR QUÉ UN GASTO NEGATIVO Y NO UNA TABLA APARTE
-- ============================================================
--
-- Con las transferencias la respuesta fue una tabla propia, porque una
-- transferencia hay que EXCLUIRLA del flujo y una tabla aparte lo vuelve
-- imposible de contar mal.
--
-- Acá es al revés: una devolución tiene que ENTRAR en el flujo, restando. Si
-- viviera en otra tabla, cada una de las veintitrés consultas que suman gastos
...
--
-- Guardada como un gasto de monto negativo, `sum(monto) filter (tipo='gasto')`
-- la resta sola. En las veintitrés. Sin tocar ninguna.
--
-- El desglose por destino la resta de su categoría, la trayectoria del saldo la
-- suma de vuelta, la conciliación cuadra, el presupuesto se libera. Todo eso
-- sin una línea de código nuevo, que es la señal de que el modelo es el
-- correcto y no un parche.
--
-- ============================================================
-- NEGATIVO SOLO EN GASTOS
-- ============================================================
--
-- Un ingreso negativo no significa nada —o entró plata o no entró— y admitirlo
-- abriría la puerta a que un error de signo se guarde como si fuera un dato.
-- La restricción nueva dice exactamente eso: los gastos pueden ser negativos,
-- el resto no.

alter table public.eos_movimientos_financieros
  drop constraint if exists eos_movimientos_financieros_monto_check;

alter table public.eos_movimientos_financieros
  add constraint eos_movimientos_financieros_monto_check
  check (
    -- Un gasto negativo es una devolución: plata que había salido y volvió.
    (tipo = 'gasto' and monto <> 0)
    or (tipo <> 'gasto' and monto >= 0)
  );

comment on column public.eos_movimientos_financieros.monto is
  'Positivo siempre, salvo un gasto NEGATIVO: eso es una devolución. Se guarda así para que las 23 consultas que suman gastos la resten solas, sin que ninguna tenga que acordarse.';

-- ============================================================
-- La acción existente aprende a devolver
-- ============================================================
--
-- No hay acción nueva: `REGISTRAR_MOVIMIENTO_PERSONAL` acepta
-- `tipo = 'devolucion'` y lo guarda como el gasto negativo. Una acción aparte
-- sería redundante —los datos son los mismos: cuánto, en qué, cuándo— y cada
-- acción nueva son cuatro lugares en n8n y tres `check` que alguien tiene que
-- acordarse de tocar.
...
begin
  ...
  loop
    ...
    insert into public.eos_movimientos_financieros (
      ...
      v_moneda,
      left(case when v_tipo = 'devolucion' then 'Devolución — ' || v_descripcion else v_descripcion end, 300),
      v_fecha,
...
comment on function public.eos_finanzas_registrar_personal_v136(uuid, uuid, jsonb) is
  'v141: anota gastos, ingresos y DEVOLUCIONES de la persona. La devolución se guarda como gasto negativo para que reste del gasto de su categoría en vez de inflar los ingresos.';
