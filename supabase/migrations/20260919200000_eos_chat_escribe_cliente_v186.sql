-- v186: el chat le escribe a un cliente ("sí, escribile") por el WhatsApp de la empresa.
--
-- ============================================================
-- EL HUECO
-- ============================================================
--
-- El CRM ya avisa a quién hay que retomar y propone qué decirle. Pero la respuesta
-- natural a "Marcos no contesta hace 4 días, ¿le escribo esto?" es "sí, escribile" en
-- el chat, y el chat no podía: no existía el verbo. La persona tenía que ir a la
-- pantalla, copiar el texto y mandarlo a mano.
--
-- ============================================================
-- QUÉ HACE Y QUÉ NO
-- ============================================================
--
-- Agrega ENVIAR_WHATSAPP_CLIENTE en los CUATRO lugares de la base donde se da de alta
-- una acción del chat: los tres CHECK y la lista blanca del ejecutor.
--
-- La función nueva solo VALIDA: cliente único, mensaje, canal activo, teléfono y que no
-- haya pedido la baja. El envío lo hace el servidor con el mismo código que la pantalla,
-- porque la política de WhatsApp (ventana de 24 horas, plantillas, tope diario) tiene
-- que ser una sola. Por eso el ejecutor devuelve `envio: { estado: 'pendiente' }` y el
-- servidor lo completa con lo que Meta contestó.
--
-- ============================================================
-- QUÉ SE COPIÓ DE LA BASE, Y POR QUÉ
-- ============================================================
--
-- El ejecutor se reescribe ENTERO a partir de su definición viva
-- (`pg_get_functiondef`), no de una migración vieja: la base va por delante de la rama
-- y generarlo desde un archivo viejo revertiría verbos ya desplegados. Lo único que
-- cambia respecto de la v182 es la rama nueva, la lista blanca y `executor_version`.

-- ------------------------------------------------- validar antes de escribirle a un cliente

create or replace function public.eos_crm_preparar_envio_por_chat_v186(
  p_usuario_id uuid,
  p_command_id uuid,
  p_datos jsonb
)
...
declare
  ...
  v_canal_id uuid;
  v_canal_estado text;
  v_canal_secreto text;
begin
  if not public.eos_tiene_modulo(p_usuario_id, 'crm') then
    raise exception 'EOS_ACCION_SIN_MODULO_CRM';
  end if;

  v_texto_cliente := nullif(btrim(coalesce(
    p_datos ->> 'cliente', p_datos ->> 'contacto', p_datos ->> 'nombre', ''
  )), '');

  v_mensaje := nullif(btrim(coalesce(
    p_datos ->> 'mensaje', p_datos ->> 'texto', ''
  )), '');

  if v_texto_cliente is null then
    raise exception 'EOS_ACCION_WHATSAPP_SIN_CLIENTE';
  end if;

  if v_mensaje is null then
    raise exception 'EOS_ACCION_WHATSAPP_SIN_MENSAJE';
  end if;

  -- El límite de WhatsApp es 4096; uno de 1000 ya no es un seguimiento, es una carta.
  if char_length(v_mensaje) > 1000 then
    raise exception 'EOS_ACCION_WHATSAPP_MENSAJE_LARGO';
  end if;

  /*
   * El cliente TIENE que existir y ser uno solo. `eos_crm_resolver_contacto` devuelve
   * null si no hay coincidencia o si hay dos con el mismo nombre parcial: en ese caso
   * NO se elige uno. Un mensaje al cliente equivocado no se puede deshacer.
   */
  v_contacto_id := public.eos_crm_resolver_contacto(p_usuario_id, v_texto_cliente);

  if v_contacto_id is null then
    raise exception 'EOS_ACCION_CONTACTO_NO_RESUELTO: %', v_texto_cliente;
  end if;
  ...
  if v_telefono is null or btrim(v_telefono) = '' then
    raise exception 'EOS_ACCION_WHATSAPP_SIN_TELEFONO: %', v_nombre;
  end if;

  select k.id, k.estado, k.secreto_ref
    into v_canal_id, v_canal_estado, v_canal_secreto
  from public.eos_wa_canales k
  where k.usuario_id = p_usuario_id
    and k.estado <> 'desconectado'
  order by k.creado_en
  limit 1;

  if v_canal_id is null or v_canal_secreto is null then
    raise exception 'EOS_ACCION_WHATSAPP_SIN_CANAL';
  end if;

  if v_canal_estado <> 'activo' then
    raise exception 'EOS_ACCION_WHATSAPP_CANAL_PAUSADO';
  end if;

  -- Quien pidió la baja no recibe nada: ni de la persona ni de EOS.
  if exists (
    select 1
    from public.eos_wa_consentimientos k
    where k.usuario_id = p_usuario_id
      and k.contacto_id = v_contacto_id
      and k.estado = 'revocado'
  ) then
    raise exception 'EOS_ACCION_WHATSAPP_BAJA: %', v_nombre;
  end if;
  ...
  return jsonb_build_object(
    ...
    'canal_id', v_canal_id,
    'mensaje', v_mensaje,
    -- El servidor completa esto después de hablar con Meta.
    'envio', jsonb_build_object('estado', 'pendiente')
  );
end;
$function$;

revoke all on function public.eos_crm_preparar_envio_por_chat_v186(uuid, uuid, jsonb)
  from public, anon, authenticated;
grant execute on function public.eos_crm_preparar_envio_por_chat_v186(uuid, uuid, jsonb)
  to service_role;
...
begin
  ...
  if v_command.accion not in (
    ...
    'ANULAR_COMPRA', 'CORREGIR_COMPRA', 'ENVIAR_WHATSAPP_CLIENTE'
  ) then
  ...
  elsif v_command.accion = 'ENVIAR_WHATSAPP_CLIENTE' then
    v_effect_type := 'whatsapp_cliente';

    /*
     * ACÁ SOLO SE VALIDA; EL ENVÍO LO HACE EL SERVIDOR.
     *
     * Mandarle un mensaje a un cliente es hablar con un tercero por el canal de la
     * empresa: hay que llamar a Meta con el token de Vault, aplicar la política de
     * la ventana de 24 horas, la baja del cliente y el tope diario, y registrar el
     * mensaje con su motivo si no salió. Todo eso vive en el código que también usa
     * la pantalla (`lib/whatsapp-crm/enviar.ts`), y duplicarlo acá sería tener dos
     * políticas distintas para el mismo mensaje.
     *
     * Lo que SÍ se resuelve acá, antes de dar la orden por cumplida, son los
     * errores que la persona puede corregir en la misma frase: a qué cliente, qué
     * decirle, que haya canal, que tenga teléfono y que no haya pedido la baja.
     *
     * No crea nada por su cuenta, así que un reintento no duplica nada; el envío
     * es idempotente por la clave del comando.
     */
    v_rpc := public.eos_crm_preparar_envio_por_chat_v186(
      v_command.usuario_id,
      v_command.id,
      v_data
    );

    v_effect_id := (v_rpc ->> 'contacto_id')::uuid;
    v_result := coalesce(v_result, '{}'::jsonb) || v_rpc;
...
revoke all on function public.eos_execute_internal_effect_v64(uuid) from public, anon, authenticated;
grant execute on function public.eos_execute_internal_effect_v64(uuid) to service_role;
...
begin
  ...
  from (values
    ...
    ('ENVIAR_WHATSAPP_CLIENTE', 'accion_ejecutada', 'preparó el envío de un mensaje a un cliente por WhatsApp', 'preparar el envío de un mensaje a un cliente por WhatsApp'),
