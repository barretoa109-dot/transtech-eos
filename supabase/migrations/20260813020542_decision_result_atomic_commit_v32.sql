...
revoke all on function public.eos_record_decision_result_v32(uuid, uuid, text, text, text, text, text, numeric) from public, anon, service_role;
grant execute on function public.eos_record_decision_result_v32(uuid, uuid, text, text, text, text, text, numeric) to authenticated;
...
left join lateral (
  select
    ...
    (array_agg(dr.tipo order by dr.ocurrido_at desc, dr.created_at desc))[1] as latest_result_type,
    (array_agg(dr.resumen order by dr.ocurrido_at desc, dr.created_at desc))[1] as latest_result_summary,
...
comment on function public.eos_record_decision_result_v32(uuid, uuid, text, text, text, text, text, numeric) is
  'RC1 v32: registra resultado y evaluación de decisión de forma atómica e idempotente por request_id.';

comment on view public.eos_decision_registry_v6 is
  'Registro consolidado de decisiones, resultados, aprendizaje y evaluación EOS actualizada.';
