...
begin
  ...
  select
    ...
    case
      when coalesce(r.metadata->>'source_revision', '') ~ '^[0-9]+$'
        then (r.metadata->>'source_revision')::bigint
      else null
    end
  into
    v_existing_changed,
    v_existing_fingerprint,
    v_existing_source_revision
  ...
  if found then
    ...
    if v_context.source_fingerprint is distinct from v_existing_fingerprint then
      raise exception 'EOS_CONTEXT_IDEMPOTENCY_SUPERSEDED';
    end if;
  ...
  if found then
    ...
    v_version := case
      when v_changed then v_context.version + 1
      else v_context.version
    end;
  end if;
  ...
  insert into public.eos_master_context_runs (
    ...
    jsonb_build_object(
      ...
      'request_fingerprint_binding', true
    )
  );
...
comment on function public.eos_commit_master_context_v33(
  ...
  'RC1 v56: commit atomico de Contexto Maestro con source revision guard y binding estricto de request_id + fingerprint + revision para replays.';