...
-- Cerrar políticas heredadas de lectura pública. Todas estas tablas contienen
-- datos por usuario; los clientes autenticados solo deben ver filas propias.
...
create index if not exists eos_kpis_usuario_created_idx_v19
  on public.eos_kpis (usuario_id, created_at desc);
...
create index if not exists eos_tendencias_usuario_created_idx_v19
  on public.eos_tendencias (usuario_id, created_at desc);
...
create index if not exists recomendaciones_usuario_created_idx_v19
  on public.recomendaciones (usuario_id, created_at desc);
...
create index if not exists score_historico_usuario_created_idx_v19
  on public.score_historico (usuario_id, created_at desc);

-- Legacy notifications stores owner as text.
...
create index if not exists notificaciones_usuario_created_idx_v19
