...
begin
  ...
  -- Fase 4 auto-starts a fresh command but does not assign a fencing token.
  -- That initial lease is not Worker ownership and keeps the same attempt.
  v_attempt := case
...
begin
  ...
  -- The historical terminal error trigger stores error metadata but not result.
...
do $$
begin
  if to_regprocedure('public.eos_claim_action_command_v64(uuid,integer)') is not null then
    execute 'revoke all on function public.eos_claim_action_command_v64(uuid, integer) from public, anon, authenticated, service_role';
  end if;
  if to_regprocedure('public.eos_renew_action_command_lease_v64(uuid,integer,integer)') is not null then
    execute 'revoke all on function public.eos_renew_action_command_lease_v64(uuid, integer, integer) from public, anon, authenticated, service_role';
  end if;
  if to_regprocedure('public.eos_finish_action_command_v64(uuid,integer,boolean,jsonb,text,text)') is not null then
    execute 'revoke all on function public.eos_finish_action_command_v64(uuid, integer, boolean, jsonb, text, text) from public, anon, authenticated, service_role';
  end if;
  if to_regprocedure('public.eos_finish_action_command_v65(uuid,uuid,integer,boolean,jsonb,text,text)') is not null then
    execute 'revoke all on function public.eos_finish_action_command_v65(uuid, uuid, integer, boolean, jsonb, text, text) from public, anon, authenticated, service_role';
  end if;
...
comment on function public.eos_claim_action_command_v65(uuid, integer) is
  'RC1 v71: canonical authorized claim with unique lease_token fencing and durable claim marker.';
comment on function public.eos_renew_action_command_lease_v65(uuid, uuid, integer, integer) is
  'RC1 v71: renews only the current live authorized fenced lease.';
comment on function public.eos_finalize_action_command_v70(uuid, uuid, integer, text, jsonb, text, text) is
  'RC1 v71: terminal result fenced by lease_token + attempt, authorization, durable claim and exact replay binding.';
