-- Que una venta no se pierda por un producto que no está en el catálogo (v156).
--
-- Se genera desde el archivo de la v154 —el ejecutor vivo— con cuatro cambios
-- y nada más:
--
--   1. Cuatro variables nuevas en el bloque `declare`.
--   2. El lazo de ítems de REGISTRAR_VENTA llama a
--      `eos_erp_resolver_o_crear_producto` en vez de resolver y rendirse.
--   3. La venta deja en `resultado` qué productos se crearon y de cuáles falta
--      el costo, para que el chat pueda decirlo.
--   4. `executor_version` pasa a 'v156'.
--
-- El porqué está entero en la cabecera de
-- `20260910150000_eos_resolver_variantes_v156.sql`. En una línea: el 9 de
-- septiembre de 2026 una usuaria vendió un conjunto verde oliva talle S, EOS
-- entendió todo y no registró nada, porque el producto no estaba cargado y la
-- venta era todo-o-nada.
--
-- LO QUE NO CAMBIA: si hay VARIOS productos parecidos, sigue fallando y
-- preguntando cuál. Crear uno nuevo ahí partiría el catálogo en dos.
...
begin
  ...
  if v_command.accion not in (
    ...
    'REGISTRAR_TARJETA', 'REGISTRAR_COMPRA_TARJETA', 'REGISTRAR_OPORTUNIDAD'
  ) then
