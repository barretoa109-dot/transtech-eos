-- Las ramas de REGISTRAR_DEUDA y REGISTRAR_PAGO_DEUDA en el ejecutor.
--
-- Se genera desde el archivo de la v138 con tres cambios y nada más:
--
--   1. Las dos acciones entran en la lista de soportadas.
--   2. Las dos ramas nuevas.
--   3. `executor_version` pasa a 'v139'.
--
-- El porqué está en la cabecera de
-- `20260908060000_eos_deudas_por_chat_v139.sql`: el motor de deudas más útil
-- del producto estaba detrás de la barrera de entrada más alta, un
-- formulario por acreedor.
--
-- Ojo con la idempotencia del pago: la marca es el movimiento que dejó,
-- porque un reintento descontaría el saldo dos veces.
