...
-- Canonical runtime lifecycle for non-transactional/external effects.
-- Internal DB effects keep using eos_execute_internal_effect_v64 because their
-- unique action_command_id binding is already atomic in Postgres.
...
begin
  ...
  -- The historical Fase 4 start trigger may move a fresh row to ejecutando but
  -- it never assigns lease_token. Such a row has not been formally claimed.
  v_attempt := case
...
begin
  ...
  if not exists (
    select 1 from public.eos_action_events e
    where e.command_id = v_command.id
      and e.usuario_id = v_command.usuario_id
...
begin
  ...
  if not exists (
    select 1 from public.eos_autonomy_events_v12 e
    where e.command_id = v_command.id
      and e.usuario_id = v_command.usuario_id
      and e.event_type in ('auto_allowed', 'consumed')
  ) then
  ...
  -- Exact terminal replay stays idempotent even after the lease has been
  -- cleared. An incompatible replay is always rejected.
  ...
  -- Fase 4's terminal error trigger stores error metadata but not resultado.
...
grant execute on function public.eos_claim_action_command_v65(uuid, integer)
  to service_role;
grant execute on function public.eos_renew_action_command_lease_v65(uuid, uuid, integer, integer)
  to service_role;
