...
begin
  if p_usuario_id is null then raise exception 'EOS_PAYMENT_USER_REQUIRED'; end if;
  if v_plan_codigo not in ('personal', 'pro', 'business') then raise exception 'EOS_PAYMENT_PLAN_INVALID'; end if;
  if v_periodicidad not in ('mensual', 'anual') then raise exception 'EOS_PAYMENT_PERIOD_INVALID'; end if;
  if p_comprador is null or jsonb_typeof(p_comprador) <> 'object' then raise exception 'EOS_PAYMENT_BUYER_REQUIRED'; end if;
  if p_cuenta_destino is null or jsonb_typeof(p_cuenta_destino) <> 'object' then raise exception 'EOS_PAYMENT_DESTINATION_REQUIRED'; end if;
  ...
  select case when v_periodicidad = 'anual' then p.precio_anual_pyg else p.precio_mensual_pyg end::bigint
    into v_monto
  from public.planes p
  where p.codigo = v_plan_codigo and p.activo = true and p.es_publico = true
  limit 1;
  if v_monto is null or v_monto <= 0 then raise exception 'EOS_PAYMENT_PLAN_PRICE_INVALID'; end if;
  ...
  set estado = 'vencido',
      ...
      metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
        'expired_at', now(),
        'expired_by', 'eos_create_or_reuse_transfer_request_v49'
      )
  ...
  limit 1
  for update;
  if found then raise exception 'EOS_PAYMENT_ALREADY_IN_REVIEW'; end if;
  ...
  if found then
    ...
    insert into public.solicitudes_pago (
      usuario_id, plan_codigo, periodicidad, moneda, monto, proveedor, estado,
      referencia_interna, vencimiento_pago, metadata
    ) values (
      p_usuario_id, v_plan_codigo, v_periodicidad, 'PYG', v_monto, 'transferencia',
      'pendiente_transferencia', v_reference, now() + interval '48 hours',
      jsonb_build_object(
        'comprador', p_comprador,
        'cuenta_destino', p_cuenta_destino,
        'request_creation_version', 'v49'
...
comment on function public.eos_create_or_reuse_transfer_request_v47(uuid, text, text, jsonb, jsonb) is
  'RC1 v49: crea o reutiliza solicitud pendiente serializada; bloquea nueva compra del mismo plan/periodicidad mientras exista comprobante en revisión y repite el guard antes de insertar para cerrar la carrera con uploads.';
