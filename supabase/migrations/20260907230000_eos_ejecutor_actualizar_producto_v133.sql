-- La rama de ACTUALIZAR_PRODUCTO en el ejecutor interno.
--
-- Se genera desde el archivo de la v132 con tres cambios y nada más:
--
--   1. 'ACTUALIZAR_PRODUCTO' entra en la lista de acciones soportadas.
--   2. La rama nueva, que delega en `eos_erp_actualizar_productos_v133`.
--   3. `executor_version` pasa a 'v133'.
--
-- El porqué de la acción está en la cabecera de
-- `20260907220000_eos_actualizar_producto_v133.sql`: una usuaria le dictó a
-- EOS el costo y el margen de cuatro prendas, y todo terminó en una memoria
-- de texto porque no existía el verbo para escribirlo en el catálogo.
--
-- Como en la v131, la función se genera desde el archivo anterior en vez de
-- transcribirse: son siete ramas, y reescribirla de memoria para agregar una
-- es la forma más fácil de perder alguna de las otras seis sin que ninguna
-- prueba lo note.
