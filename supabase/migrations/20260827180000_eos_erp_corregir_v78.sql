-- Poder deshacer y corregir en el ERP (v78).
--
-- Hasta acá el ERP sabía registrar una venta, registrar una compra, cobrar y
-- pagar. Nada más. Una venta cargada mal ya había descontado stock y ya había
-- movido plata, y no existía forma de revertirla: ni desde la pantalla, ni
-- desde la base.
--
-- Eso no es una función que falta, es un motivo para dejar de usar el módulo.
-- El almacenero que se equivoca el primer día y descubre que el sistema no lo
-- deja arreglarlo, no vuelve a abrirlo.
--
-- Esta migración agrega las tres operaciones que faltaban:
--
--   * anular una venta      -> devuelve el stock y saca la plata
--   * anular una compra     -> saca el stock y devuelve la plata
--   * ajustar el stock      -> conteo físico, rotura, merma
--
-- ============================================================
-- ANULAR NO ES DEVOLVER
-- ============================================================
--
-- Acá "anular" quiere decir *esta venta no debió existir*: se cargó mal, se
-- duplicó, se puso el cliente equivocado. Por eso el movimiento financiero se
-- BORRA en vez de compensarse con uno de signo contrario.
--
-- Compensar sería lo correcto para una devolución real, donde la plata sí entró
-- y después salió. Pero para un error de carga, un gasto compensatorio le
-- mostraría al usuario una salida de dinero que nunca ocurrió, y el panel
-- volvería a mentir — que es exactamente lo que EOS existe para no hacer.
--
-- La devolución de mercadería es otro hecho económico y merece su propio
-- tratamiento el día que se necesite. No se disfraza de anulación.
--
-- ============================================================
-- EL STOCK SOLO SE MUEVE DEJANDO RASTRO
-- ============================================================
--
-- Ninguna de estas funciones escribe `stock_actual` en silencio: cada cambio
-- inserta su fila en `eos_erp_movimientos_stock` con el saldo resultante. Esa
-- es la única razón por la que dentro de seis meses se va a poder explicar por
-- qué el sistema dice 12 y en el estante hay 9.
--
-- Por lo mismo, la edición de un producto (que agrega esta misma tanda, del
-- lado de la API) NO puede tocar `stock_actual`: para eso está el ajuste.
...
begin
  ...
  loop
    ...
    elsif v_item.costo_anterior is not null then
      update public.eos_erp_productos
