-- El contexto del chat también mezclaba las dos plata.
--
-- `eos_contexto_negocio` es lo que EOS recibe antes de contestar. Su bloque
-- de finanzas sumaba TODOS los movimientos del mes sin mirar el ámbito, así
-- que el modelo leía "entró X, salió Y" con la venta del negocio y el sueldo
-- de la persona en el mismo renglón.
--
-- La v136 separó la plata en la base y en las pantallas; esto es la misma
-- separación en lo que ve el modelo. Sin esto, EOS escribe en el lugar
-- correcto y después contesta sobre la mezcla.
--
-- `finanzas` pasa a ser el negocio y aparece `personal`. La forma de cada
-- uno no cambia —una fila por moneda, como desde la v94— así que el código
-- que ya lee `finanzas` sigue funcionando, ahora con el número correcto.
--
-- Se genera desde el archivo de la v94 en vez de transcribirse.
