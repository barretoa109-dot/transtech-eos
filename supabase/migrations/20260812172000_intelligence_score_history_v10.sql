...
create table if not exists public.eos_intelligence_score_snapshots_v10 (
  ...
  strongest_dimension text not null
    check (strongest_dimension in ('contexto', 'objetivos', 'ejecucion', 'decisiones', 'aprendizaje')),
  weakest_dimension text not null
    check (weakest_dimension in ('contexto', 'objetivos', 'ejecucion', 'decisiones', 'aprendizaje')),
...
create unique index if not exists eos_intelligence_score_user_day_uidx
  on public.eos_intelligence_score_snapshots_v10 (usuario_id, snapshot_day);

create index if not exists eos_intelligence_score_user_history_idx
  on public.eos_intelligence_score_snapshots_v10 (usuario_id, snapshot_day desc);

alter table public.eos_intelligence_score_snapshots_v10 enable row level security;

drop policy if exists eos_intelligence_score_select_own
  on public.eos_intelligence_score_snapshots_v10;
create policy eos_intelligence_score_select_own
  on public.eos_intelligence_score_snapshots_v10
...
drop policy if exists eos_intelligence_score_insert_own
  on public.eos_intelligence_score_snapshots_v10;
create policy eos_intelligence_score_insert_own
  on public.eos_intelligence_score_snapshots_v10
...
drop policy if exists eos_intelligence_score_update_own
  on public.eos_intelligence_score_snapshots_v10;
create policy eos_intelligence_score_update_own
  on public.eos_intelligence_score_snapshots_v10
...
comment on table public.eos_intelligence_score_snapshots_v10 is
  'Historial diario autenticado del EOS Intelligence Score para explicar tendencia, variaciones y señales que movieron el score.';
comment on column public.eos_intelligence_score_snapshots_v10.formula_version is
  'Version de la formula utilizada para evitar comparar silenciosamente scores calculados con metodologias incompatibles.';
