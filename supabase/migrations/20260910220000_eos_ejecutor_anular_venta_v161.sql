-- La rama de ANULAR_VENTA en el ejecutor (v161).
--
-- Se genera desde el ejecutor vivo con tres cambios y nada más:
--
--   1. La acción entra en la lista de soportadas.
--   2. La rama nueva.
--   3. `executor_version` pasa a 'v161'.
--
-- El porqué está en la cabecera de
-- `20260910210000_eos_anular_venta_por_chat_v161.sql`. En una línea: la
-- función para anular existe desde el 27 de agosto y no había forma de
-- llegar a ella hablando; el 9 de septiembre una usuaria cargó mal una venta
-- por chat y tuvo que ir a la pantalla a deshacerla a mano.
--
-- La idempotencia es distinta a la de las demás. Las otras acciones marcan la
-- fila que crearon; ésta no crea ninguna, anula una que ya existía. La marca
-- es `anulada_por_command_id` y la mira la función, no esta rama.
...
begin
  ...
  if v_command.accion not in (
    ...
    'REGISTRAR_TARJETA', 'REGISTRAR_COMPRA_TARJETA', 'REGISTRAR_OPORTUNIDAD', 'ANULAR_VENTA'
  ) then
