-- Las ramas de REGISTRAR_TARJETA y REGISTRAR_COMPRA_TARJETA en el ejecutor.
--
-- Se genera desde el archivo de la v151 con tres cambios y nada más:
--
--   1. Las dos acciones entran en la lista de soportadas.
--   2. Las dos ramas nuevas.
--   3. `executor_version` pasa a 'v153'.
--
-- El porqué está en la cabecera de
-- `20260910100000_eos_tarjetas_por_chat_v153.sql`: la vertical de tarjetas se
-- construyó entera el 8 de septiembre y se cargaba toda a mano, justo donde
-- más duele — la compra en cuotas se hace en la caja de un negocio, con el
-- teléfono en la mano, y si no se anota ahí no se anota nunca.
