-- EOS — Una orden que la regla de negocio rechaza se cierra con su motivo (v195)
--
-- Escrita el 12/09/2026 como v164 en otra carpeta y nunca subida ni aplicada;
-- rescatada el 23/09 como v195 (el número v164 ya lo usa
-- 20260914000000_eos_emails_transaccionales_v164). Al 23/09 el bug seguía:
-- `cerrarConMotivo` todavía llamaba a v66 y recibía 42501.
--
-- ============================================================
...
-- ============================================================
--
-- `eos_cerrar_orden_rechazada_v195` hace una sola cosa: cerrar con `error` una
...
create or replace function public.eos_cerrar_orden_rechazada_v195(
  p_command_id uuid,
  p_error_code text,
...
comment on function public.eos_cerrar_orden_rechazada_v195(uuid, text, text) is
  'v195: cierra con error una orden que nunca se tomó con lease, cuando la regla de negocio la rechazó. Delega en eos_finalize_action_command_v66. Solo service_role; las tomadas con lease se cierran con v70.';

revoke all on function public.eos_cerrar_orden_rechazada_v195(uuid, text, text) from public, anon, authenticated;
grant execute on function public.eos_cerrar_orden_rechazada_v195(uuid, text, text) to service_role;
