-- La rama de CORREGIR_VENTA en el ejecutor (v162).
--
-- Se genera desde el ejecutor vivo con tres cambios y nada más:
--
--   1. La acción entra en la lista de soportadas.
--   2. La rama nueva.
--   3. `executor_version` pasa a 'v162'.
--
-- El porqué está en la cabecera de
-- `20260910240000_eos_corregir_venta_por_chat_v162.sql`. En una línea:
-- `eos_erp_editar_venta` existe desde el 4 de septiembre y no había forma de
-- llegar a ella hablando, así que corregir un número mal entendido obligaba
-- a anular y redictar la venta entera.
--
-- La idempotencia es obligatoria acá y no opcional: corregir CREA una venta
-- nueva, así que un reintento sin marca dejaría tres ventas donde había una.
