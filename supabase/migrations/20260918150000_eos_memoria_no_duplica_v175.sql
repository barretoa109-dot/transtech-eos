-- Guardar una memoria que ya existe la reobserva; no la duplica.
--
-- ============================================================
-- QUÉ SE MIDIÓ
-- ============================================================
--
-- El 2026-09-18, en las cuentas reales, 5 de las 11 memorias tenían el mismo
-- título que otra del mismo usuario: cada vez que la persona repetía un dato
-- ("mi meta es...", "vendo por Instagram"), EOS guardaba OTRA fila. El
-- contexto que recibe el modelo se llena de repeticiones, y ninguna de las
-- copias sabe cuántas veces se confirmó. La idempotencia que había era solo
-- por COMANDO (el mismo comando reintentado no duplica), no por CONTENIDO.
--
-- `eos_memory` ya tenía las columnas para esto —`observaciones` y
-- `ultima_observacion_at`— y el ejecutor nunca las usaba.
--
-- ============================================================
-- QUÉ CAMBIA
-- ============================================================
--
-- SOLO la rama GUARDAR_MEMORIA de `eos_execute_internal_effect_v64`. Antes de
-- insertar busca una memoria ACTIVA del mismo usuario con el mismo título
-- normalizado (minúsculas, espacios colapsados):
--
--   * si existe, la reobserva: `observaciones + 1`, `ultima_observacion_at`,
--     y si el contenido cambió gana el nuevo y el anterior queda en
--     `metadata.contenido_anterior`. Devuelve `idempotent = true`: no se
--     creó un efecto nuevo.
--   * si no existe (o solo existe archivada: una memoria que la persona
--     archivó no se revive), inserta como siempre.
--
-- Un reintento del mismo comando no vuelve a contar: los comandos que ya
-- reobservaron una memoria quedan en `metadata.comandos`. Y un advisory lock
-- por usuario+título evita que dos comandos simultáneos creen las dos.
--
-- Solo se compara el título EXACTO normalizado, a propósito: unir memorias
-- "parecidas" es adivinar, y una memoria fusionada de más es un dato de la
-- persona perdido. No se tocan las 5 duplicadas que ya existen.
--
-- El resto de la función es texto idéntico al desplegado (verificado con
-- `pg_get_functiondef` antes de escribir esto). `create or replace` conserva
-- los permisos: no se vuelve a tocar ningún grant.
