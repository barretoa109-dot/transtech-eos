...
begin
  ...
  select r.usuario_id, r.created_at
    into v_run_owner, v_run_created_at
  ...
  if exists (
    select 1
    from public.eos_document_intelligence_runs_v11 newer
    where newer.document_id = p_document_id
      and newer.usuario_id = v_uid
      and newer.id <> p_run_id
      and newer.created_at > v_run_created_at
  ) then
    raise exception 'EOS_STALE_ANALYSIS_RUN';
  end if;
...
comment on function public.eos_commit_document_analysis_v29(uuid, uuid, text, jsonb, jsonb, jsonb, jsonb) is
  'Fase 8 hardening v29/v30: commit documental atómico self-scoped; solo el run más reciente del documento puede reemplazar la inteligencia vigente.';
