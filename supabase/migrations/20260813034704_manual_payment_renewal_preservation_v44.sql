  ...
  if v_action = 'aprobar' then
    ...
    if v_same_active_plan then
      ...
      set plan_inicio = coalesce(v_previous_start, now()),
          plan_vencimiento = v_previous_expiry + make_interval(months => v_months),
    ...
    set estado = 'pagado',
        ...
        metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
          ...
          'procesado_at', now(),
          'manual_payment_atomic_version', 'v44',
          'same_plan_renewal', v_same_active_plan,
          'meses_acreditados', v_months
        )
...
comment on function public.eos_process_manual_payment_v42(uuid, text, text) is
  'RC1 v44: pago manual atómico/idempotente con lock de solicitud+usuario; renovación del mismo plan vigente extiende desde vencimiento actual sin perder tiempo pagado.';