...
begin
  ...
  loop
    ...
    loop
      ...
      if evidence_type not in ('decision_result', 'goal_event', 'action_event')
         or evidence_id = '' then
      ...
      if nullif(canonical_item ->> 'ocurrido_at', '') is not null then
        v_first_observed_at := least(
          coalesce(v_first_observed_at, (canonical_item ->> 'ocurrido_at')::timestamptz),
          (canonical_item ->> 'ocurrido_at')::timestamptz
        );
        v_last_observed_at := greatest(
          coalesce(v_last_observed_at, (canonical_item ->> 'ocurrido_at')::timestamptz),
          (canonical_item ->> 'ocurrido_at')::timestamptz
        );
      end if;
    end loop;
    ...
    loop
      insert into public.eos_learning_evidence (
        ...
        case when canonical_item ->> 'signal' in
          ('positivo', 'neutral', 'negativo', 'inconcluso')
          then canonical_item ->> 'signal' else 'neutral' end,
