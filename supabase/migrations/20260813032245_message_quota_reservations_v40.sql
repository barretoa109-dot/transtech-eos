...
begin
  ...
  if p_request_id is null then
    raise exception 'EOS_MESSAGE_REQUEST_ID_REQUIRED';
  end if;
  ...
  if v_plan.id is null then
    raise exception 'EOS_MESSAGE_PLAN_INVALID';
  end if;
...
begin
  ...
  if not found then
    return jsonb_build_object('ok', true, 'status', 'missing', 'idempotent', true);
  end if;
...
begin
  ...
  if v_plan = 'free' then
    ...
    v_result := jsonb_set(
      v_result,
      '{disponibilidad,puede_enviar_mensajes}',
      to_jsonb(v_active and (v_limit is null or (v_used + v_reserved) < v_limit)),
      true
    );
...
begin
  ...
  select lower(coalesce(u.plan, 'free')), p.limite_mensajes
    into v_plan, v_limit
  from public.usuarios u
  join public.planes p on p.codigo = lower(coalesce(u.plan, 'free'))
