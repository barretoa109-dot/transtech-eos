...
create index if not exists eos_business_twins_updated_idx on public.eos_business_twins_v14 (updated_at desc);
...
create unique index if not exists eos_business_twin_snapshots_user_version_uidx on public.eos_business_twin_snapshots_v14 (usuario_id, version);
create index if not exists eos_business_twin_snapshots_user_created_idx on public.eos_business_twin_snapshots_v14 (usuario_id, created_at desc);
...
create trigger eos_business_twin_touch_v14 before update on public.eos_business_twins_v14 for each row execute function public.eos_touch_business_twin_v14();
...
select twin.*,
  ...
  greatest(0,floor(extract(epoch from (now()-twin.generated_at))/60))::integer as age_minutes
from public.eos_business_twins_v14 twin;
...
create policy eos_business_twins_select_own_v14 on public.eos_business_twins_v14 for select to authenticated using ((select auth.uid())=usuario_id);
create policy eos_business_twin_snapshots_select_own_v14 on public.eos_business_twin_snapshots_v14 for select to authenticated using ((select auth.uid())=usuario_id);
...
comment on table public.eos_business_twins_v14 is 'Modelo operativo estructurado y versionado de la realidad del usuario/empresa. Se deriva de fuentes EOS existentes; no reemplaza el Contexto Maestro.';
comment on table public.eos_business_twin_snapshots_v14 is 'Historial inmutable por versión del Business Twin para comparar cómo cambia la realidad operativa.';
comment on view public.eos_business_twin_current_v14 is 'Business Twin vigente con indicadores de frescura derivados.';
