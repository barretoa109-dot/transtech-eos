-- El catálogo del chat no llevaba el stock.
--
-- ============================================================
-- EL CASO QUE LO EXPUSO
-- ============================================================
--
-- Una usuaria con 3 unidades de un producto cargadas en la App le preguntó a
-- EOS por WhatsApp cuántas tenía, y EOS no supo contestar. No es un problema
-- de memoria ni de que el chat y la App usen datos distintos: es que el
-- catálogo que viaja al modelo en cada mensaje (v158/v160, `lib/eos/
-- contexto-negocio.ts`) solo lleva nombre y precio. El único lugar donde
-- aparece una cantidad es `bajo_minimo`, y ese bloque se arma con máximo 5
-- productos que ya estén en o por debajo de su stock mínimo — un producto con
-- stock sano, como este, nunca entra ahí.
--
-- Reproducido llamando a la función contra un usuario con productos reales:
-- el campo `stock` no existe en ningún elemento de `erp.catalogo`.
--
-- ============================================================
-- LO QUE CAMBIA
-- ============================================================
--
-- Se agrega `stock` al catálogo, solo para productos con `controla_stock`
-- activo — para los que no lo tienen, la pregunta "cuántos tengo" no tiene
-- una respuesta que EOS pueda dar con certeza, y mandar un cero inventado
-- sería peor que no mandar nada. `lib/eos/contexto-negocio.ts` (textoCatalogo)
-- se actualiza en el mismo cambio para imprimirlo.
--
-- Se genera desde la definición que corre en producción (confirmada llamando
-- a `pg_get_functiondef` antes de escribir esto), tocando solo el bloque del
-- catálogo: el resto es texto idéntico al desplegado, incluida la corrección
-- de deudas de otra sesión (`coalesce(d.estado, 'al_dia') <> 'saldada'`) que
-- el archivo de la v161 en el repo todavía no reflejaba.
