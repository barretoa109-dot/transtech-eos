-- Las ramas de REGISTRAR_COBRO y REGISTRAR_PAGO_COMPRA en el ejecutor.
--
-- Se genera desde el archivo de la v149 con tres cambios y nada más:
--
--   1. Las dos acciones entran en la lista de soportadas.
--   2. Las dos ramas nuevas, que difieren en un booleano.
--   3. `executor_version` pasa a 'v151'.
--
-- El porqué está en la cabecera de
-- `20260909130000_eos_cobro_y_pago_por_chat_v151.sql`: la cuenta corriente
-- estaba construida desde el 2 de septiembre y envejecía sola porque marcar un
-- cobro obligaba a salir del chat.
--
-- La idempotencia va sobre `eos_erp_cuenta_movimientos_v107`, que desde la
-- v151 lleva `action_command_id`. Es la más importante de todas las que hay
-- en este archivo: un reintento sin marca cobraría el mismo documento dos
-- veces y dejaría el saldo del cliente en menos, que es un error que se lee
-- como un anticipo y nadie revisa.
