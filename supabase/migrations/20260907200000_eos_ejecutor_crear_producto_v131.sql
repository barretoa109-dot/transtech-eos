-- La rama de CREAR_PRODUCTO en el ejecutor interno.
--
-- Esta migración reescribe `eos_execute_internal_effect_v64` a partir del
-- cuerpo EXACTO que dejó la v84, con tres cambios y nada más:
--
--   1. 'CREAR_PRODUCTO' entra en la lista de acciones soportadas.
--   2. La rama nueva, que delega en `eos_erp_crear_productos_v131`.
--   3. `executor_version` pasa a 'v131'.
--
-- Se genera desde el archivo de la v84 y no se transcribe a mano: es una
-- función larga con seis ramas, y reescribirla de memoria para agregar una
-- es la forma más fácil de perder alguna de las otras cinco sin que ninguna
-- prueba lo note.
