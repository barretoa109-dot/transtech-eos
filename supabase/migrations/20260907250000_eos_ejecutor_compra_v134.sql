-- Las ramas de REGISTRAR_COMPRA y REGISTRAR_GASTO_FIJO en el ejecutor.
--
-- Se genera desde el archivo de la v133 con tres cambios y nada más:
--
--   1. Las dos acciones entran en la lista de soportadas.
--   2. Las dos ramas nuevas, que delegan en
--      `eos_erp_registrar_compra_chat_v134` y
--      `eos_finanzas_registrar_fijo_v134`.
--   3. `executor_version` pasa a 'v134'.
--
-- El porqué está en la cabecera de
-- `20260907240000_eos_registrar_compra_v134.sql`: EOS no tenía ninguna
-- acción para la plata que SALE, que en un negocio que arranca es todo lo
-- que pasa.
--
-- Como en la v131 y la v133, la función se genera desde el archivo anterior
-- en vez de transcribirse: son nueve ramas, y reescribirla de memoria para
-- agregar dos es la forma más fácil de perder alguna de las otras siete sin
