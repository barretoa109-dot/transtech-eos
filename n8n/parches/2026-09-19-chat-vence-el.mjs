 ...
 *
 *     SECO=1 node n8n/parches/2026-09-19-chat-vence-el.mjs   (prueba: no escribe)
 *     node n8n/parches/2026-09-19-chat-vence-el.mjs          (escribe)
 *
 * gateway: el prompt (nodo HTTP Request).
 * worker:  las frases de venta y compra (nodo "05 INT Respuesta").
 *
 * El porqué está en `cambios-chat-vence-el.mjs` y en la migración v182.
 *
 * ORDEN: la migración v182 se aplica ANTES que esto. Si el prompt empieza a mandar
 * `vence_dia` y el ejecutor todavía no lo entiende, el vencimiento se pierde sin
 * ningún aviso. Y este parche necesita que el de fechas legibles ya esté aplicado.
 *
 * Después de aplicar:
 *
...
import { aplicarPrompt, aplicarWorker } from "./cambios-chat-vence-el.mjs";
