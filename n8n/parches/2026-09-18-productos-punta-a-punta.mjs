/**
 * Reglas del prompt para que cargar el negocio por chat no termine en una nota
 * ni en una orden que falla.
 *
 *     SECO=1 node n8n/parches/2026-09-18-productos-punta-a-punta.mjs   (prueba)
 *     node n8n/parches/2026-09-18-productos-punta-a-punta.mjs          (escribe)
 *
 * Toca UN solo lugar: el prompt del nodo HTTP Request del gateway. No hay
 * acciones nuevas. El porqué está entero en cambios-productos-punta-a-punta.mjs.
 *
...
import { aplicar } from "./cambios-productos-punta-a-punta.mjs";
...
if (http.parameters.jsonBody.includes("AJUSTAR_STOCK y ACTUALIZAR_PRODUCTO son DISTINTAS")) {
  throw new Error("El prompt ya tiene las reglas de productos sin catálogo. No se escribió nada.");
}
...
if (process.env.SECO === "1") {
  ...
  console.log("gateway actualizado: reglas de productos sin catálogo y listas sin precio.");
