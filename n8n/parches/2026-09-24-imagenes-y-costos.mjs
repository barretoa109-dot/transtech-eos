/**
 * EOS entiende lo que ya tiene enfrente (24/09/2026), del lado de n8n.
 *
 *     SECO=1 node n8n/parches/2026-09-24-imagenes-y-costos.mjs   (prueba: no escribe)
 *     node n8n/parches/2026-09-24-imagenes-y-costos.mjs          (escribe)
 *
 * Solo el gateway:
 *   - "06 GW Preparar Jobs Worker": un ACTUALIZAR_PRODUCTO que solo pone costo
 *     corre después de la venta o compra que crea el producto
 *     (`cambios-orden-costo.mjs`).
 *   - "HTTP Request" (el prompt): lo que ya está en la conversación —una
 *     imagen anterior, "estos"— se usa en vez de preguntarlo de nuevo
 *     (`cambios-contexto-conversacion.mjs`).
 *
 * Va junto con la migración v197 (lecturas de imágenes), pero no depende de
 * ella. Después de aplicar:
 *
...
import { aplicar } from "./cambios-orden-costo.mjs";
import { aplicarPrompt } from "./cambios-contexto-conversacion.mjs";
...
g06.parameters.jsCode = aplicar(g06.parameters.jsCode, "06 GW Preparar Jobs Worker");
