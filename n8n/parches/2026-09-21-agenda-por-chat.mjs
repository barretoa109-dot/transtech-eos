 ...
 *
 *     SECO=1 node n8n/parches/2026-09-21-agenda-por-chat.mjs   (prueba: no escribe)
 *     node n8n/parches/2026-09-21-agenda-por-chat.mjs          (escribe)
 *
 * gateway: el prompt (nodo HTTP Request) y los campos de fecha de CREAR_TAREA
 *          (nodo "06 GW Preparar Jobs Worker").
 * worker:  la frase de la tarea (nodo "05 INT Respuesta").
 *
 * El porqué está en `cambios-agenda-por-chat.mjs` y en la migración v189.
 *
 * ORDEN: la migración v189 se aplica ANTES que esto. Si el prompt empieza a mandar
 * `vence_dia` y el ejecutor todavía no lo entiende, la fecha se pierde sin ningún
 * aviso. Este parche necesita además que el de fechas legibles ya esté aplicado.
 *
 * Después de aplicar:
 *
...
import { aplicarJobs, aplicarPrompt, aplicarWorker } from "./cambios-agenda-por-chat.mjs";
