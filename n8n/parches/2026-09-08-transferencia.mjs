/**
 * Dar de alta REGISTRAR_TRANSFERENCIA.
 *
...
import { aplicar } from "./cambios-transferencia.mjs";
...
// ------------------------------------------------------------------- nodo 05
const BLANCA_VIEJA = `    'REGISTRAR_GASTO_FIJO',
    'REGISTRAR_MOVIMIENTO_PERSONAL'
  ]);`;

const BLANCA_NUEVA = `    'REGISTRAR_GASTO_FIJO',
    'REGISTRAR_MOVIMIENTO_PERSONAL',
    'REGISTRAR_TRANSFERENCIA'
  ]);`;

// ------------------------------------------------------------------- nodo 06
const RUTA_VIEJA = `  REGISTRAR_MOVIMIENTO_PERSONAL: 'eos-worker-rc1-internal'
};`;

const RUTA_NUEVA = `  REGISTRAR_MOVIMIENTO_PERSONAL: 'eos-worker-rc1-internal',

  /*
    Plata que se mueve entre cuentas propias. Mismo camino: lo que cambia es la
    tabla donde aterriza, que a propósito NO es la de movimientos.
  */
...
if (http.parameters.jsonBody.includes("REGISTRAR_TRANSFERENCIA")) {
  throw new Error("El prompt ya menciona REGISTRAR_TRANSFERENCIA. No se escribió nada.");
}
