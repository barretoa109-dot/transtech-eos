/**
 * Que el chat pueda cargar una tarjeta y una compra en cuotas.
 *
 *     node n8n/parches/2026-09-10-tarjetas.mjs
 *
 ...
 * `2026-09-10-tarjetas-worker.mjs`.
 *
 * El porqué está en la migración v153: la vertical se construyó entera el 8 de
 * septiembre y se cargaba toda a mano, justo donde más duele — la compra en
 * cuotas se hace en la caja de un negocio, con el teléfono en la mano.
 */
...
import { aplicar } from "./cambios-tarjetas.mjs";
...
const BLANCA_NUEVA = `    'REGISTRAR_COBRO',
    ...
    /*
      Las tarjetas. La compra en cuotas NO escribe un movimiento financiero: lo
      que sale del bolsillo es el pago del resumen, no la compra. Anotarla como
      gasto contaría la misma plata siete veces.
    */
    'REGISTRAR_TARJETA',
    'REGISTRAR_COMPRA_TARJETA'
  ]);`;

const RUTA_VIEJA = `  REGISTRAR_PAGO_COMPRA: 'eos-worker-rc1-internal'
};`;

const RUTA_NUEVA = `  REGISTRAR_PAGO_COMPRA: 'eos-worker-rc1-internal',
...
if (http.parameters.jsonBody.includes("REGISTRAR_TARJETA")) {
  throw new Error("El prompt ya menciona REGISTRAR_TARJETA. No se escribió nada.");
}
