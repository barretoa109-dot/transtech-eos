/**
 * Que el chat pueda cobrar una venta y pagar una compra.
 *
 *     node n8n/parches/2026-09-09-cobro-y-pago.mjs
 *
 ...
 * `2026-09-09-cobro-y-pago-worker.mjs`.
 *
 * El porqué está en la migración v151: la cuenta corriente estaba construida
 * desde el 2 de septiembre —con cobros parciales, bloqueo de fila y su
 * reverso— y envejecía sola porque marcar un cobro obligaba a salir del chat.
 */
...
import { aplicar } from "./cambios-cobro-y-pago.mjs";
...
const BLANCA_NUEVA = `    'DECLARAR_SALDO',
    ...
    /*
      Cobrar una venta y pagar una compra. Son la misma operación con el signo
      cambiado, y las dos escriben en DOS lados: bajan el saldo del documento y
      dejan el movimiento financiero.

      Un cobro mal cargado no solo ensucia la caja: deja una factura como
      cobrada, y la persona deja de reclamarla.
    */
    'REGISTRAR_COBRO',
...
// ------------------------------------------------------------------- nodo 06
const RUTA_VIEJA = `  DECLARAR_SALDO: 'eos-worker-rc1-internal'
};`;

const RUTA_NUEVA = `  DECLARAR_SALDO: 'eos-worker-rc1-internal',

  /*
    Las dos de cartera van por el camino interno como el resto. Lo que las
    distingue —que recorren la cartera del contacto imputando de la factura
    más vieja a la más nueva— lo resuelve la función de la base, no la ruta.
  */
...
if (http.parameters.jsonBody.includes("REGISTRAR_COBRO")) {
  throw new Error("El prompt ya menciona REGISTRAR_COBRO. No se escribió nada.");
}
