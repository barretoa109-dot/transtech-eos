/**
 * El motor del chat de EOS, sin nada de HTTP.
 *
 * ============================================================
 * POR QUÉ ESTO NO VIVE EN `app/api/eos/route.ts`
 * ============================================================
 *
 * Hasta el 10 de septiembre de 2026 esta lógica vivía entera adentro de la
 * ruta web. Cuando se agregó WhatsApp como segundo canal, la alternativa
 * obvia —copiarla ahí también— era exactamente el error que ya costó caro
 * una vez: la memoria del proyecto sobre el workflow de n8n dice que una
 * acción nueva se da de alta en CUATRO lugares distintos, y "la cuarta es la
 * que se olvida, y no da error visible". Un canal nuevo con su propia copia
 * de esta función hubiera sido un quinto lugar, con el mismo problema.
 *
 * Por eso esto se extrajo tal cual —no se reescribió— y ambos canales
 * (`app/api/eos/route.ts` para la web, `app/api/whatsapp/webhook/route.ts`
 * para WhatsApp) llaman a la misma función. Cualquier cambio acá afecta a
 * los dos por igual, que es la propiedad que se quería.
 *
 * ============================================================
 * QUÉ NO HACE
 * ============================================================
 *
 * No sabe nada de sesiones ni de cookies: recibe `usuarioId` ya resuelto por
 * quien llama (la sesión de Supabase en la web, el número de WhatsApp
 * vinculado en el otro canal) y usa el cliente admin para todo, con el
 * `usuario_id` explícito en cada consulta — el mismo patrón que ya usaba la
 * mayoría de este código antes de moverse.
 *
 * No arma la respuesta HTTP: devuelve `{ status, body }` y quien llama decide
 * los headers (la web manda `Cache-Control`/`Vary`; WhatsApp no).
 */
...
/** Lo que devuelve el motor: un código de estado HTTP y el cuerpo, sin headers. */
export type ResultadoProcesamiento = {
  status: number;
  body: Record<string, unknown>;
};

export type EntradaProcesamiento = {
  mensaje: string;
  archivos: ArchivoEOS[];
  /** Ya recortado por quien llama; puede ser cadena vacía. */
  ...
  requestId: unknown;
  /**
   * Nombre a usar si `usuarios.nombre` está vacío. La web arma esta cadena a
   * partir de su sesión (`user.user_metadata`, el email); este módulo no
   * sabe nada de sesiones, así que la recibe ya resuelta.
   */
...
export function textoSeguro(valor: unknown, max = 500) {
...
export function normalizarArchivo(valor: unknown): ArchivoEOS | null {
...
export async function procesarMensajeEOS(
  usuarioId: string,
  ...
  try {
    const { mensaje, archivos, cita: citaEntrada } = entrada;
    ...
    const usuarioPromise: Promise<{ data: UsuarioEOS | null; error: unknown }> = Promise.resolve(
      adminSinTipos()
        ...
        .eq("id", usuarioId)
        .maybeSingle(),
    ).then(({ data, error }: { data: UsuarioEOS | null; error: unknown }) => ({ data, error }))
    ...
    const contextoPromise: Promise<ContextoNegocio | null> = Promise.resolve(
      adminSinTipos().rpc("eos_contexto_negocio", { p_usuario_id: usuarioId }),
    )
    ...
    if (conversacionId) {
      if (!esUuid(conversacionId)) {
        return {
          status: 400,
          body: { respuesta: "La conversación indicada no es válida." },
        };
      }

      const { data: conversacion, error: conversationError } = await adminSinTipos()
        .from("conversaciones")
        .select("id")
      ...
      if (!conversacion) {
        return {
          status: 403,
          body: { respuesta: "La conversación no pertenece a tu sesión actual." },
        };
      }
    }
    ...
    const nombreServidor =
      ...
      textoSeguro(entrada.nombreFallback, 120) ||
      "Usuario";
     ...
     */
    const citaCruda = (citaEntrada ?? null) as Record<string, unknown> | null;
    ...
    const payload = {
      ...
      historial: normalizarHistorial(entrada.historial),
    ...
    if (process.env.N8N_EOS_INTERNAL_SECRET) {
      n8nHeaders["x-eos-internal-secret"] = process.env.N8N_EOS_INTERNAL_SECRET;
    }
    ...
    if (n8nResponse === null) {
      try {
        n8nResponse = await fetch(N8N_EOS_URL, {
          method: "POST",
          headers: n8nHeaders,
    ...
    if (!n8nResponse.ok) {
       ...
       */
      console.error("Error desde n8n:", resumenDeRespuesta(n8nResponse.status, rawText));
      await releaseQuota(`n8n_http_${n8nResponse.status}`);

      return {
        status: n8nResponse.status >= 400 && n8nResponse.status < 600 ? n8nResponse.status : 502,
        body: {
          respuesta:
