import type { FinancialPersistencePlanV1_3 } from "./critical-sources-persistence";
import {
  parseFinancialPersistenceRpcResponse,
  type FinancialPersistenceRpcResponse,
} from "./supabase-persistence-store";

export interface FinancialPersistenceRpcClientV1_3 {
  rpc(
    functionName: "eos_financial_persist_snapshot_v1_3",
    args: {
      p_usuario_id: string;
      p_batch: FinancialPersistencePlanV1_3;
    },
  ): Promise<{
    data: unknown;
...
/** Server-only adapter for the v1.3 critical-source coverage extension. */
export class SupabaseFinancialPersistenceStoreV1_3 {
  constructor(
    private readonly rpcClient: FinancialPersistenceRpcClientV1_3,
    private readonly trustedUserId: string,
  ) {
    if (!trustedUserId) throw new Error("financial_persistence_missing_trusted_user");
  }

  async persist(plan: FinancialPersistencePlanV1_3): Promise<FinancialPersistenceRpcResponse> {
    if (
    ...
    const { data, error } = await this.rpcClient.rpc(
      "eos_financial_persist_snapshot_v1_3",
      {
