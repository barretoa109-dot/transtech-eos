...
-- Security model:
--   * service/server only;
--   * the browser must never call this function directly;
--   * p_usuario_id must come from the authenticated server boundary, never from client JSON;
--   * all persistence happens in the transaction of one PostgreSQL function invocation;
--   * Financial Context is inserted last.
...
begin
  if p_usuario_id is null then
    raise exception using
      errcode = '22023',
      message = 'financial_persistence_missing_user';
  end if;

  if p_batch is null or jsonb_typeof(p_batch) <> 'object' then
    raise exception using
      errcode = '22023',
      message = 'financial_persistence_invalid_batch';
  end if;
  ...
  if v_version <> 'financial-persistence-plan-v1' then
    raise exception using
      errcode = '22023',
      message = 'financial_persistence_unsupported_version';
  end if;

  if nullif(p_batch ->> 'userId', '')::uuid is distinct from p_usuario_id then
    raise exception using
      errcode = '42501',
      message = 'financial_persistence_user_mismatch';
  end if;
  ...
  if v_provider_key is null then
    raise exception using
      errcode = '22023',
      message = 'financial_persistence_missing_provider';
  end if;
  ...
  if v_context is null or jsonb_typeof(v_context) <> 'object' then
    raise exception using
      errcode = '22023',
      message = 'financial_persistence_missing_context';
  end if;

  v_context_revision := nullif(v_context ->> 'revision', '');
  v_context_fingerprint := nullif(v_context ->> 'sourceFingerprint', '');
  if v_context_revision is null or v_context_fingerprint is null then
    raise exception using
      errcode = '22023',
      message = 'financial_persistence_invalid_context_identity';
  end if;

  -- Exact context replay is a no-op. This check happens before mutable source rows
  -- so an exact retry cannot generate incidental writes or timestamps.
  select *
  ...
  if found then
    if v_existing_context.revision <> v_context_revision then
      raise exception using
        errcode = '23505',
        message = 'financial_context_fingerprint_revision_mismatch';
    end if;
  ...
  -- 1. Connection state. Credentials/tokens are intentionally not part of this payload.
  for v_connection in
    select value from jsonb_array_elements(coalesce(p_batch -> 'connectionUpserts', '[]'::jsonb))
  loop
  ...
  for v_account in
    select value from jsonb_array_elements(coalesce(p_batch -> 'accountUpserts', '[]'::jsonb))
  loop
  ...
  -- 3. Immutable ingestion. Same source event + different fingerprint is a hard error.
  for v_event in
    select value from jsonb_array_elements(coalesce(p_batch -> 'ingestionEventUpserts', '[]'::jsonb))
  loop
    ...
    if found then
      if v_existing_event.source_fingerprint <> v_event ->> 'sourceFingerprint'
         or v_existing_event.payload_hash <> md5(v_event ->> 'payloadFingerprintMaterial') then
        raise exception using
          errcode = '23505',
          message = 'financial_ingestion_replay_mismatch';
      end if;
    else
      insert into public.eos_financial_ingestion_events_v1 (
        ...
        md5(v_event ->> 'payloadFingerprintMaterial'),
  ...
  -- 4. Canonical Ledger. Upsert by canonical key to allow lifecycle advancement
  -- without creating a second economic effect.
  for v_ledger in
    select value from jsonb_array_elements(coalesce(p_batch -> 'ledgerUpserts', '[]'::jsonb))
  loop
    ...
    select a.id
      into v_account_id
      from public.eos_financial_accounts_v1 a
      join public.eos_financial_connections_v1 c on c.id = a.connection_id
     where a.usuario_id = p_usuario_id
       and c.provider_key = v_provider_key
    ...
    if v_account_id is null or v_source_event_id is null then
    ...
    if found then
      -- PL/pgSQL cannot distinguish INSERT from UPDATE here without extra machinery.
      -- This counter is defined as rows materially touched by the statement; exact context
      -- replay exits above before reaching this loop.
      v_ledger_touched := v_ledger_touched + 1;
    end if;
  end loop;

  -- Resolve explicit reversal links after every canonical Ledger row exists.
  for v_ledger in
    select value from jsonb_array_elements(coalesce(p_batch -> 'ledgerUpserts', '[]'::jsonb))
  loop
  ...
  for v_reconciliation in
    select value from jsonb_array_elements(coalesce(p_batch -> 'reconciliationInserts', '[]'::jsonb))
  loop
    ...
    select array_agg(l.id order by l.canonical_key)
    ...
    if coalesce(array_length(v_reconciliation_entry_ids, 1), 0)
       <> jsonb_array_length(v_reconciliation -> 'ledgerCanonicalKeys') then
    ...
    if found then
      v_reconciliation_touched := v_reconciliation_touched + 1;
    end if;
  end loop;

  -- 6. Recurrences, with canonical Ledger evidence resolved to UUIDs.
  for v_recurrence in
    select value from jsonb_array_elements(coalesce(p_batch -> 'recurrenceUpserts', '[]'::jsonb))
  loop
    ...
    if coalesce(array_length(v_recurrence_source_ids, 1), 0)
       <> jsonb_array_length(v_recurrence -> 'sourceLedgerCanonicalKeys') then
  ...
  -- 7. Obligations.
  for v_obligation in
    select value from jsonb_array_elements(coalesce(p_batch -> 'obligationUpserts', '[]'::jsonb))
  loop
  ...
  -- 8. Context is the commit marker and therefore comes last.
...
--   * validate this function against a non-production Supabase branch/project;
--   * add transaction/race/replay-mismatch tests using real Postgres;
--   * verify service_role ownership/grants in the actual Supabase environment;
--   * decide whether payload hashes should use pgcrypto digest(..., 'sha256') instead of md5;
--   * audit SECURITY DEFINER with Supabase Security Advisor;
--   * rehearse rollback;
--   * do not apply to production until RC1 freeze is closed.
